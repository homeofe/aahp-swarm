> Note (2026-07-19, claude-opus-4-8): Aligned the AAHP v3.8.0 conformance PR (#10) with its code review. Restored the task-tracking metadata the CLI manifest regen had stripped (next_task_id and tasks T-001/T-002 with their github_issue links) and restored the MANIFEST project field to aahp-swarm. Left the canonical GROUNDING.md and TRUST.md provenance wording unchanged: those files are copied verbatim from the @elvatis_com/aahp package by init/migrate-grounding, so per-repo edits would fork the canonical template and churn on the next tooling run. Corrected the PR-body follow-up that claimed GitHub Actions was off org-wide (cost sweep): Actions is enabled for this repo and the AAHP Verify workflow already runs on PRs.

> Note (2026-07-18, claude-opus-4-8): Adopted CLI-based AAHP conformance (v3.8.0). Added .ai/handoff/GROUNDING.md and a Provenance section to TRUST.md (fixes the grounding gate); regenerated MANIFEST.json so next_task_id is an integer (fixes the manifest-schema gate). Rewired .github/workflows/aahp-verify.yml to run the published CLI (npx @elvatis_com/aahp verify/doctor) instead of the vendored bash scripts, and removed the vendored AAHP toolchain (scripts/_aahp-lib.sh, aahp-manifest.sh, lint-handoff.sh, verify-handoff.sh, install-hooks.sh, verify-hooks.sh, scripts/hooks/). This repo has no root package.json, so the pinned-dep, changelog-format, and version-sync gates stay skip (opt-in). scripts/aahp-archive.sh was left in place pending a maintainer decision.
